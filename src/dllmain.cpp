...
DWORD WINAPI worker_thread(LPVOID)
{
    ...
    while (!g_stop.load()) {
        if (GetModuleHandleA("samp.dll")) break;
    ...
    for (int attempt = 1; attempt <= 120 && !g_stop.load(); ++attempt) {
        if (rakhook::initialize()) {
            ...
            ss << "RakHook initialized. attempt=" << attempt
               << " samp_version=" << static_cast<int>(rakhook::samp_version());
        ...
        if (attempt == 120) {
            intercept::log::error("RakHook initialization failed after 120 attempts.");
            intercept::console::print("[ERROR] RakHook initialization failed.");
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(250));
    }
